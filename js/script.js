function __nexusMain() {
    ...
    let productNotifHistory = [];
    let productNotifLastSeen = parseInt(localStorage.getItem('jv_notif_lastseen') || '0');
...
<span class="c-author uname uname-${nameFx}" onclick="location.href='perfil.html?u=${c.authorId}'" style="cursor:pointer;${userFxColor ? ` --uname-color: ${userFxColor};` : ''}">${esc(c.author)}${nPlusHTML}</span>
    ...
    if (!window.fb || !window.fb.getComments || !window.fb.updateComment) {
        toast('❌ Firebase no disponible', 'error');
        return;
    }
    const commentsResult = await window.fb.getComments();
    if (!commentsResult.success) {
        toast('Error al obtener comentarios', 'error');
        return;
    }
    const c = commentsResult.data.find(x => x.id === id);
    if (!c) {
        toast('Comentario no encontrado', 'error');
        return;
    }
    if (c.authorId !== currentUser.uid) {
        toast('No puedes editar este comentario', 'error');
        return;
    }

    // Rellenar el modal y mostrarlo
    document.getElementById('editCommentId').value = id;
    document.getElementById('editCommentText').value = c.text;
...
async function saveEditComment() {
    ...
    if (!text) {
        toast('El comentario no puede estar vacío', 'error');
        return;
    }
    if (!window.fb || !window.fb.updateComment) {
        toast('❌ Firebase no disponible', 'error');
        return;
    }
    const result = await window.fb.updateComment(id, 'text', text, 'set');
    if (result.success) {
        await loadComments();
        toast('✅ Comentario editado');
        closeOverlay('editCommentOverlay');
    } else {
...
function renderProducts() {
    ...
    grid.innerHTML = displayList.map((p, i) => {
        ...
        const updateBadge = p.updateStatus === 'updated'
            ? `<span class="prod-update-tag updated"><i class="fas fa-check-circle"></i> Actualizado</span>`
            : p.updateStatus === 'outdated'
            ? `<span class="prod-update-tag outdated"><i class="fas fa-exclamation-triangle"></i> Desactualizado</span>`
            : '';
...
function renderProductsShowMoreButton(totalCount) {
    ...
    window.openProd = function(id) {
        ...
        // Mostrar badge de actualización en el modal
let updateBadgeModal = document.getElementById('mUpdateBadge');
if (!updateBadgeModal) {
    updateBadgeModal = document.createElement('span');
    updateBadgeModal.id = 'mUpdateBadge';
    document.getElementById('mCategory').insertAdjacentElement('afterend', updateBadgeModal);
}
if (currentProd.updateStatus === 'updated') {
    updateBadgeModal.className = 'prod-update-tag updated';
    updateBadgeModal.style.display = 'inline-flex';
    updateBadgeModal.innerHTML = '<i class="fas fa-check-circle"></i> Actualizado';
} else if (currentProd.updateStatus === 'outdated') {
    updateBadgeModal.className = 'prod-update-tag outdated';
    updateBadgeModal.style.display = 'inline-flex';
    updateBadgeModal.innerHTML = '<i class="fas fa-exclamation-triangle"></i> Desactualizado';
} else {
    updateBadgeModal.style.display = 'none';
}
        // Productos marcados como nexusPlusFree son gratis para usuarios con Nexus+
        const unlockedByNexusPlus = !!(currentProd.nexusPlusFree && currentUser && currentUser.hasNexusPlus);

        const displayPrice = currentProd.free ? 'Gratis' : (unlockedByNexusPlus ? 'Gratis con Nexus+' : convertPrice(currentProd.price, currentCurrency));
        ...
        if (currentProd.richHTML) {
            ...
            const rawFeats = currentProd.feats || [];
            // ✅ Detecta datos corruptos guardados ANTES del fix (array partido letra por letra)
            const isCorruptedFeats = rawFeats.length > 6 && rawFeats.every(f => typeof f === 'string' && f.length <= 2);
            if (isCorruptedFeats) {
                $('mFeats').innerHTML = `<li style="display:flex;align-items:center;gap:10px;font-size:.83rem;color:var(--text-faint);"><i class="fas fa-triangle-exclamation" style="color:var(--amber);width:14px;font-size:.75rem;"></i>Las características de este producto quedaron guardadas con un formato dañado (dato antiguo). Edítalo desde el panel y guarda de nuevo para corregirlo.</li>`;
            } else {
                $('mFeats').innerHTML = rawFeats.map(f =>
        ...
        if (currentProd.free || unlockedByNexusPlus) {
            $('mPaySection').style.display = 'none';
            $('mFreeSection').style.display = '';
            const downloadBtn = $('mDownloadBtn');
            if (currentUser && unlockedByNexusPlus) {
                // Producto premium desbloqueado gratis por ser Nexus+: no hay archivo
                // directo (beta cerrada), así que en vez de descargar se abre el modal
                // con los pasos para reclamarlo por Discord.
                downloadBtn.href = 'javascript:void(0)';
                downloadBtn.innerHTML = '<i class="fas fa-gem"></i> Descargar Gratis (Nexus+)';
                downloadBtn.className = 'btn btn-solid btn-block';
                downloadBtn.style.background = 'var(--plus-grad, linear-gradient(135deg,#ffe9b8,#ffb347,#b98cff))';
                downloadBtn.style.border = 'none';
                downloadBtn.onclick = function(e) {
                    e.preventDefault();
                    openNexusProClaimModal(currentProd);
                };
            } else if (currentUser) {
            ...
            // Aviso para quien no es Nexus+ de que este producto es gratis con esa membresía
            let plusHint = document.getElementById('mPlusHint');
            if (currentProd.nexusPlusFree) {
                if (!plusHint) {
                    plusHint = document.createElement('div');
                    plusHint.id = 'mPlusHint';
                    plusHint.style.cssText = 'display:flex;align-items:center;gap:10px;background:rgba(255,179,71,.07);border:1px solid rgba(255,179,71,.25);border-radius:12px;padding:12px 14px;margin-bottom:14px;font-size:.8rem;color:var(--text-dim);';
                    $('mPaySection').prepend(plusHint);
                }
                plusHint.style.display = 'flex';
                plusHint.innerHTML = '<i class="fas fa-gem" style="color:var(--plus-gold-2,#ffb347);font-size:1rem;"></i> <span><strong style="color:var(--text);">Es gratis si eres Nexus+.</strong> Actívalo desde tu perfil para descargar esta app sin pagar.</span>';
            } else if (plusHint) {
    ...
    // ============================================================
    // MODAL: RECLAMAR PRODUCTO GRATIS POR NEXUS+ (ej. Nexus Pro)
    // ============================================================
    window.openNexusProClaimModal = function(product) {
        const p = product || currentProd;
        if (!p) return;
        const nameEl = document.getElementById('claimProductName');
        if (nameEl) nameEl.textContent = p.name;
        const joinBtn = document.getElementById('claimJoinServerBtn');
        if (joinBtn) joinBtn.href = p.dl && p.dl.startsWith('http') ? p.dl : 'https://discord.com/invite/RBBXNZaNDw';

        closeOverlay('prodOverlay');
        setTimeout(() => {
            document.getElementById('nexusProClaimOverlay').classList.add('open');
            document.body.style.overflow = 'hidden';
        }, 150);
    };

    window.copyLegalesCommand = function() {
        const cmd = '/legales';
        const finish = () => toast('Comando /legales copiado');
        if (navigator.clipboard && navigator.clipboard.writeText) {
            navigator.clipboard.writeText(cmd).then(finish).catch(() => {
                toast('Copia manualmente: /legales', 'error');
            });
        } else {
            toast('Copia manualmente: /legales', 'error');
        }
...
function showSaveOrderButton() {
    ...
    if (currentUser) {
        ...
        if (pnb) pnb.onclick = () => { location.href = 'perfil.html'; };
...
function initLangButton() {
    ...
    function initCaptcha() {
        if (!document.getElementById('captchaOverlay')) return;
    ...
    function initBackToTop() {
        const __btt = document.getElementById('backToTop'); if (!__btt) return;
        ...
        backBtn.onclick = () => {
            ...
            setTimeout(() => backBtn.classList.remove('launching'), 650);
        };
    }
    ...
    function initMusicPlayer() {
        ...
        ($('mpAddUrlBtn')||{}).onclick = () => {
            const urlInput = $('mpUrlInput'), nameInput = $('mpUrlNameInput'), statusEl = $('mpUrlStatus');
            if (urlInput) urlInput.value = '';
            if (nameInput) nameInput.value = '';
            if (statusEl) { statusEl.textContent = ''; statusEl.className = 'status-msg'; }
            const overlay = $('mpUrlOverlay');
            if (overlay) {
                overlay.classList.add('open');
                document.body.style.overflow = 'hidden';
            }
        ...
        // Expuesto para que el modal #mpUrlOverlay (fuera de este closure) pueda
        // agregar la pista a esta playlist sin necesitar acceso directo a ella.
        window.__mpAddTrackByUrl = (url, name) => {
            playlist.push({ name: name || 'Pista externa', artist: 'Link externo', src: url, removable: true });
            renderPlaylist();
        };
...
window.createNewProduct = async function() {
    ...
    if (editId) {
        ...
        if (result.success) {
            ...
            if (window.fb.addActivityNotification) {
                window.fb.addActivityNotification(currentUser.uid, currentUser.username, 'newproduct', name, result.id || customId || '');
            }
            window.cancelEditProduct();
            await loadProductsFromFirebase();
            await loadAdminData();
...
window.saveDevProduct = async function() {
  ...
  const productData = {
    ...
    desc,
    feats,
    downloadLink: downloadLink,   // ← campo específico para descarga
    buyLink: buyLink,             // ← campo específico para compra
    free: isFree,
    updateStatus, 
    authorId: currentUser.uid,
  ...
  try {
    ...
    if (result.success) {
      ...
      if (!editId && window.fb.addActivityNotification) {
        window.fb.addActivityNotification(currentUser.uid, currentUser.username, 'newproduct', name, result.id || customId || '');
      }
      cancelEditDevProduct();
      await loadProductsFromFirebase();
      await loadMyDevProducts();
    } else {
...
window.addEventListener('fb-ready', () => {
    ...
    window.fb.listenActivityNotifications((notif) => {
        ...
        if (notif.actionType === 'newproduct') handleNewProductNotif(notif);
    });
});
...
function showActivityNotification(notif) {
    ...
    } else if (notif.actionType === 'newproduct') {
        iconHtml = '<div style="width:32px;height:32px;border-radius:50%;background:rgba(61,220,151,.15);color:var(--success);display:grid;place-items:center;font-size:1rem;"><i class="fas fa-box"></i></div>';
        actionText = `publicó <strong style="color:var(--success);">${esc(notif.productName)}</strong>`;
    }
...
// ✅ Fecha relativa corta para el panel de notificaciones ("hace 3h", "hace 2d"...)
function timeAgo(ts) {
    if (!ts) return '';
    const diff = Date.now() - ts;
    const min = 60000, hr = 3600000, day = 86400000, week = 604800000;
    if (diff < min) return 'justo ahora';
    if (diff < hr) return `hace ${Math.floor(diff / min)}m`;
    if (diff < day) return `hace ${Math.floor(diff / hr)}h`;
    if (diff < week) return `hace ${Math.floor(diff / day)}d`;
    if (diff < week * 5) return `hace ${Math.floor(diff / week)}sem`;
    return new Date(ts).toLocaleDateString('es-ES', { day: '2-digit', month: '2-digit', year: 'numeric' });
}

// ✅ Convierte un color hex de producto (p.color) a rgba con alpha, para íconos con acento por marca
...
// Rellena el panel con los productos que ya existen (para que nunca salga vacío),
// evitando duplicar los que ya llegaron como notificación en vivo.
function seedProductNotifFromProducts() {
    if (!PRODUCTS || !PRODUCTS.length) return;
    const existingIds = new Set(productNotifHistory.map(n => n.productId));
    let added = false;
    PRODUCTS.forEach(p => {
        if (!p.id || existingIds.has(p.id)) return;
        productNotifHistory.push({
            productId: p.id,
            productName: p.name,
            username: p.author || p.authorName || '157 Team',
            actionType: 'newproduct',
            _ts: parseDDMMYYYY(p.updatedDate) || (p.createdAt || 0)
        });
        existingIds.add(p.id);
        added = true;
    });
    if (!added) return;
    productNotifHistory.sort((a, b) => b._ts - a._ts);
    if (productNotifHistory.length > 30) productNotifHistory.length = 30;
    renderProductNotifPanel();
    updateNotifBadge();
}

function handleNewProductNotif(notif) {
    const ts = notif.createdAt || notif.timestamp || Date.now();
    productNotifHistory.unshift({ ...notif, _ts: ts });
    productNotifHistory.sort((a, b) => b._ts - a._ts);
    if (productNotifHistory.length > 30) productNotifHistory.length = 30;
    renderProductNotifPanel();
    updateNotifBadge();
...
function updateNotifBadge() {
    ...
    if (!dot) return;
    const unread = productNotifHistory.filter(n => n._ts > productNotifLastSeen).length;
    dot.style.display = unread > 0 ? 'block' : 'none';
}

function renderProductNotifPanel() {
    const list = document.getElementById('productNotifList');
    if (!list) return;
    if (!productNotifHistory.length) {
        list.innerHTML = `<div style="text-align:center;padding:40px 20px;color:var(--text-faint);">
            <i class="fas fa-box-open" style="font-size:2rem;display:block;margin-bottom:10px;"></i>
            <p style="font-size:.85rem;">Aún no hay productos nuevos</p>
        </div>`;
        return;
    }
    list.innerHTML = productNotifHistory.map(n => {
        const prod = PRODUCTS.find(p => p.id === n.productId);
        const status = prod ? prod.updateStatus : null;

        const badge = status === 'updated'
            ? `<span class="pn-badge pn-badge--updated"><i class="fas fa-check-circle"></i> Actualizado</span>`
            : status === 'outdated'
            ? `<span class="pn-badge pn-badge--outdated"><i class="fas fa-exclamation-triangle"></i> Desactualizado</span>`
            : `<span class="pn-badge pn-badge--unset"><i class="fas fa-circle-minus"></i> Sin marcar</span>`;

        // Ícono + acento de color por producto: candado para premium, caja para gratis
        const icon = !prod ? 'fa-box' : (prod.free ? 'fa-box-open' : 'fa-lock');
        const rgbaBg = hexToRgba(prod && prod.color, .16);
        const rgbaBorder = hexToRgba(prod && prod.color, .35);
        const iconStyle = rgbaBg
            ? `background:${rgbaBg};border-color:${rgbaBorder};color:${prod.color};`
            : `background:var(--cyan-dim);border-color:var(--cyan-dim);color:var(--cyan);`;

        return `<div class="pn-item" onclick="toggleProductNotifPanel();openProd('${n.productId}')">
            <div class="pn-icon" style="${iconStyle}"><i class="fas ${icon}"></i></div>
            <div class="pn-body">
                <div class="pn-top">
                    <span class="pn-title">${esc(n.productName)}</span>
                    <span class="pn-time">${timeAgo(n._ts)}</span>
                </div>
                <div class="pn-sub">Publicado por ${esc(n.username)}</div>
                ${badge}
            </div>
        </div>`;
    }).join('');
...
window.toggleProductNotifPanel = function() {
    const panel = document.getElementById('productNotifPanel');
    if (!panel) return;
    const isOpen = panel.classList.contains('open');
    if (!isOpen) {
        panel.classList.add('open');
        renderProductNotifPanel();
        productNotifLastSeen = Date.now();
        localStorage.setItem('jv_notif_lastseen', String(productNotifLastSeen));
        updateNotifBadge();
    } else {
...
document.getElementById('dockNotifBtn')?.addEventListener('click', window.toggleProductNotifPanel);
...
function initLiquidDock() {
    const homeBtn = $('dockHomeBtn');
    if (homeBtn) homeBtn.onclick = () => window.scrollTo({ top: 0, behavior: 'smooth' });

    const createBtn = $('dockCreateBtn');
    if (createBtn) createBtn.onclick = () => {
        if (!currentUser) { openAuthModal(); return; }
        openPostCreateModal();
    };

    const chatBtn = $('dockChatBtn');
    if (chatBtn) chatBtn.onclick = () => {
        if (!currentUser) { openAuthModal(); return; }
        toggleChatPanel();
    };

    const profileBtn = $('dockProfileBtn');
