...
export const BADGE_DEFS = [
  { id: 'first_steps',    name: 'Primeros Pasos',        icon: 'fa-shoe-prints',        desc: 'Completa tu perfil por primera vez',   type: 'xp',           value: 1 },
  { id: 'chatty',         name: 'Conversador',           icon: 'fa-comments',           desc: 'Publica 10 comentarios',               type: 'comments',     value: 10 },
  { id: 'chatty_pro',     name: 'Voz de la Comunidad',   icon: 'fa-bullhorn',           desc: 'Publica 50 comentarios',               type: 'comments',     value: 50 },
  { id: 'downloader',     name: 'Explorador',            icon: 'fa-download',           desc: 'Descarga 5 productos',                 type: 'downloads',    value: 5 },
  { id: 'downloader_pro', name: 'Coleccionista',         icon: 'fa-box-open',           desc: 'Descarga 20 productos',                type: 'downloads',    value: 20 },
  { id: 'reviewer',       name: 'Crítico',               icon: 'fa-star',               desc: 'Deja 5 reseñas',                       type: 'reviews',      value: 5 },
  { id: 'poster',         name: 'Creador de Contenido',  icon: 'fa-camera',             desc: 'Publica 5 momentos en tu perfil',      type: 'profilePosts', value: 5 },
  { id: 'developer',      name: 'Desarrollador',         icon: 'fa-code',               desc: 'Publica tu primer producto',           type: 'products',     value: 1 },
  { id: 'developer_pro',  name: 'Arquitecto',            icon: 'fa-drafting-compass',   desc: 'Publica 5 productos',                  type: 'products',     value: 5 },
...
// XP acumulado necesario para llegar al nivel N (curva progresiva)
export function xpForLevel(level) {
  return Math.floor(50 * Math.pow(level, 1.55));
}
...
// Cuenta estadísticas de actividad del usuario (para evaluar insignias)
export async function getUserActivityStats(uid) {
  try {
    const [commentsSnap, reviewsSnap, postsSnap, productsSnap] = await Promise.all([
      getDocs(query(collection(db, "comments"), where("authorId", "==", uid))),
      getDocs(query(collection(db, "product_reviews"), where("userId", "==", uid))),
      getDocs(query(collection(db, "profilePosts"), where("authorId", "==", uid))),
      getDocs(query(collection(db, "products"), where("authorId", "==", uid)))
    ]);
    const userSnap = await getDoc(doc(db, "users", uid));
    const downloadsCount = userSnap.exists() ? (userSnap.data().downloadsCount || 0) : 0;
    return {
      success: true,
      stats: {
        comments: commentsSnap.size,
        reviews: reviewsSnap.size,
        profilePosts: postsSnap.size,
...
// Llamar cada vez que un usuario descarga algo. Suma XP y evalúa insignias.
export async function registerUserDownload(uid) {
  try {
    await updateDoc(doc(db, "users", uid), { downloadsCount: increment(1) });
    const statsRes = await getUserActivityStats(uid);
    return await addUserXP(uid, XP_REWARDS.download, statsRes.stats || {});
  } catch (error) {
    return { success: false, error: error.message };
  }
}
...
// ============================================================
// ESTADO PERSONALIZADO (en línea / ocupado / no molestar / custom)
// ============================================================

export async function setUserStatus(uid, mode, text = '') {
  try {
    await updateDoc(doc(db, "users", uid), {
      status: { mode, text: (text || '').substring(0, 40), updatedAt: new Date().toISOString() }
    });
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
...
// ============================================================
// MOMENTOS DE PERFIL (fotos tipo IG dentro de la ficha del usuario)
// ============================================================

export async function addProfilePost(authorId, authorName, image, caption) {
  try {
    if (!authorId || !image) return { success: false, error: "Faltan datos" };
...
export async function deleteProfilePost(postId, userId) {
  try {
...
export function listenProfilePosts(uid, callback) {
  const q = query(collection(db, "profilePosts"), where("authorId", "==", uid));
  return onSnapshot(q, (snapshot) => {
    const posts = [];
...
export async function toggleProfilePostReaction(postId, userId, reaction) {
  // reaction: 'like' | 'dislike'
  try {
    const ref = doc(db, "profilePosts", postId);
    const snap = await getDoc(ref);
    if (!snap.exists()) return { success: false, error: "No encontrado" };
    const data = snap.data();
    ...
    const already = (reaction === 'like' ? likes : dislikes).includes(userId);

    if (already) {
      await updateDoc(ref, { [field]: arrayRemove(userId) });
    } else {
      await updateDoc(ref, { [field]: arrayUnion(userId), [otherField]: arrayRemove(userId) });
    }
    return { success: true };
  } catch (error) {
    return { success: false, error: error.message };
...
export async function addProfilePostComment(postId, authorId, authorName, text) {
  try {
    if (!text || !text.trim()) return { success: false, error: "Comentario vacío" };
    const docRef = await addDoc(collection(db, "profilePostComments"), {
      postId, authorId, authorName: authorName || 'Usuario',
...
export function listenProfilePostComments(postId, callback) {
  const q = query(collection(db, "profilePostComments"), where("postId", "==", postId));
  return onSnapshot(q, (snapshot) => {
    const comments = [];
